...
MockBucket.prototype.query = function(query, params, callback) {
  ...
  } else if (query instanceof ViewQuery.Spatial) {
    throw new Error('Spatial queries are not supported in the mock.');
  } else if (query instanceof N1qlQuery.Direct) {
    throw new Error('N1QL queries are not supported in the mock.');
  } else {
    throw new TypeError(
        'First argument needs to be a ViewQuery or N1qlQuery.');
  }
};
...
MockBucket.prototype._execView = function(ddoc, name, options, callback) {
  this._indexView(ddoc, name, options, function(err, results, reducer) {
    ...
    for (var i = 0; i < results.length; ++i) {
      ...
      if (keys !== undefined) {
        // Convert keys to string in case there is composite key as an array.
        if (Array.isArray(dockey)) {
          var separator = '_';
          var dockeyStringified = dockey.join(separator);
          var keysStringified = [];
          keys.forEach(function(keyArray) {
            keysStringified.push(keyArray.join(separator));
          });
          if (keysStringified.indexOf(dockeyStringified) < 0) {
            continue;
          }
        }
        else {
